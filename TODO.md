...
4. Add a "New Buy Opportunities" tab for stocks that are not currently present in holdings or positions, including:
   - Recommended action
   - Suggested quantity to buy
5. Include a "Forecast" tab with future outlook and projections.
6. End the report with a "Prioritized Action Plan", listing recommendations in order of importance.
7. Use a "sidebar-based UI", with all report tabs accessible from the left sidebar.
8. When a stock is clicked from any tab, open a popup/modal that consolidates all analysis for that stock across every tab in a single view.
9. for non-held universe stocks check balance sheet.
10. Fetch updated news from web.